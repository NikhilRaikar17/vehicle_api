...
def add_vehicle():
    ...
    try:
        ...
        error, message = vehicle_service.add(
            db, name, brand, description, year_of_manufacture, ready_to_drive)
        if error:
...
def update_vehicle(vehicle_id):
    ...
    try:
        ...
        error, message = vehicle_service.update(
            db, vehicle_id, name, brand, description, year_of_manufacture, ready_to_drive)
        if error:
            raise Exception(message)
...
def search_vehicles():
    try:
        ...
        year_of_manufacture = request.args.getlist(
            'year_of_manufacture', type=int)
        ready_to_drive = request.form.get(
            'ready_to_drive', type=bool, default=False)

        error, message, vehicles = vehicle_service.search(
            name, brand, year_of_manufacture, ready_to_drive)
        if error:
            raise Exception(message)
